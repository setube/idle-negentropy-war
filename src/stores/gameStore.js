...
export const useGameStore = defineStore(
  ...
  () => {
    ...
    const completeEntropyStage = (stageKey) => {
      ...
      if (stageEraMap[stageKey]) {
        ...
        events.value.unshift({
          timestamp: Date.now(),
          title: '文明进化',
          description: `进入${eras.find(e => e.key === currentEra.value)?.name}！`,
          type: 'era'
        })
      }
    ...
    const unlockStageTechnologies = stageKey => {
      const stageTechMap = {
        atomicOrdering: ['atomicManipulation'],
        molecularCooling: ['thermalControl', 'quantumComputing'],
        stellarExtinction: ['stellarEngineering'],
    ...
    const updateGame = () => {
      ...
      // 更新坐标暴露值
      coordinateExposure.value += (resources.value.energy * Math.pow(civilizationLevel.value + 1, 2)) / 1000000
      ...
      // 检查黑暗森林打击
      if (coordinateExposure.value > concealmentAbility.value) {
        // 触发降维打击
        civilizationLevel.value = Math.max(0, civilizationLevel.value - 1)
    ...
    const updateCivilizationLevel = () => {
      ...
      if (newLevel > civilizationLevel.value) {
        ...
        Object.values(technologies.value).forEach(tech => {
          if (tech.unlocked) {
            tech.efficiency *= Math.exp(-0.03 * civilizationLevel.value)
          }
        })
      }
    }
